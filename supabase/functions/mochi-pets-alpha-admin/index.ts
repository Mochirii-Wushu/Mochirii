...
Deno.serve(async (req: Request) => {
  ...
  if (action === "grant" || action === "revoke") {
    ...
    const { error } = await access.adminClient.from("mochi_pets_alpha_testers").upsert(
      {
        user_id: userId,
...
async function loadAlphaAudit(adminClient: SupabaseClient) {
  const [
    ...
    safeCount(adminClient, "mochi_pets_alpha_testers", "status", "active"),
    safeCount(adminClient, "mochi_pets_alpha_testers", "status", "revoked"),
    safeCount(adminClient, "mochi_pets_ledger_events"),
    safeCount(adminClient, "mochi_pets_feedback"),
    safeCount(adminClient, "mochi_pets_chat_messages"),
    safeCount(adminClient, "mochi_pets_unity_players"),
    safeCount(adminClient, "mochi_pets_shared_pet_snapshots"),
    adminClient
      .from("mochi_pets_ledger_events")
