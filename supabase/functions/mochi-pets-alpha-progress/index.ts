...
Deno.serve(async (req: Request) => {
  ...
  if (!adminClient) {
    return jsonResponse({ ok: false, error: "mochi_pets_not_configured", message: "Mochi Pets alpha database access is not configured." }, 500);
  }
  ...
  if (!access.termsAccepted) return jsonResponse({ ok: false, error: "alpha_terms_required", message: "Alpha terms must be acknowledged before loading account progress." }, 403);

  const { data, error } = await loadAlphaProgressSnapshot(adminClient, playerId);
  if (error) {
    return jsonResponse({ ok: false, error: "progress_lookup_failed", message: "Account progress could not be loaded." }, 500);
  }
