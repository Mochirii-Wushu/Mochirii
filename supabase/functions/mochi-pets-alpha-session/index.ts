...
Deno.serve(async (req: Request) => {
  ...
  if (!state.ok) {
    return jsonResponse(
      { ok: false, error: "alpha_access_lookup_failed", message: "Mochi Pets alpha access could not be checked." },
      500,
    );
  }

  if (bodyResult.body.acknowledgeTerms === true && state.hasAccess) {
    const { error } = await access.adminClient.from("mochi_pets_terms_acknowledgements").upsert(
      {
  ...
  if (refreshed.ok && refreshed.hasAccess) {
    ...
    if (profileError) {
      return jsonResponse({ ok: false, error: "alpha_profile_failed", message: "Your Mochi Pets alpha profile could not be prepared." }, 500);
    }
  }
