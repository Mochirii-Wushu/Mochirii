...
import { CORS_HEADERS, alphaAccess, jsonResponse, readJsonBody, requireUser, safeString } from "../_shared/mochi-pets-alpha.ts";
...
Deno.serve(async (req: Request) => {
  ...
  if (!alpha.ok || !alpha.hasAccess) {
    return jsonResponse(
      { ok: false, error: "alpha_access_required", message: "Mochi Pets feedback is available to allowlisted alpha testers." },
      403,
    );
  }
  ...
  return jsonResponse({ ok: true, data, message: "Mochi Pets feedback saved." });
});
