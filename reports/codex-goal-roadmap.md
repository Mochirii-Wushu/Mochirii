...
- Status: Complete. Branch: `qa/member-gallery-end-to-end-review`. Report: `reports/member-gallery-end-to-end-review.md`. PR: <https://github.com/Mochirii-Wushu/Mochirii/pull/115>. Merge commit: `723f39bdfff813439cb3c09fec8b40c2b5ae264e`. Validation summary: member workflow source inspection, mocked browser state matrix, upload success regression repro/fix proof, `npm run check`, `git diff --check`, workflow `node --check`, JSON/JS/ref/asset checks, `npm run check:production`, `npm run smoke:gallery`, protected-data diff checks, and post-merge validation passed. Blockers: no approved live test account/session available, so positive live upload/moderation remains deferred to G09 or an approved test-account workflow. Next recommended item: G05 `qa/account-member-dashboard-review`.
...
- Status: Complete. Branch: `docs/member-gallery-moderation-runbook`. Runbook: `docs/member-gallery-moderation-runbook.md`. PR: <https://github.com/Mochirii-Wushu/Mochirii/pull/123>. Merge commit: `1c4825106a71145521c42c1897ba49de6096e0a6`. Validation summary: moderator workflow docs drafted with access prerequisites, queue handling, approval/rejection guidance, audit expectations, common errors, escalation boundaries, no-secret safety notes, `npm run check`, `git diff --check`, JSON/JS/ref/asset checks, `npm run check:production`, `npm run smoke:gallery`, protected-data diff checks, secret-pattern scan, and post-merge validation passed. Blockers: none found. Next recommended item: G09 `qa/production-member-workflow-smoke`.
...
- Status: Complete. Branch: `qa/production-member-workflow-smoke`. Report: `reports/production-member-workflow-smoke.md`. PR: <https://github.com/Mochirii-Wushu/Mochirii/pull/125>. Merge commit: `491298fe3e8355b54e4de97eb9f8844f6e073437`. Validation summary: live production signed-out Auth, Account, Gallery Submit, Leader Dashboard, Gallery, approved-feed Gallery, OAuth redirect-start, public member-submission filter, private Storage path visibility checks, `npm run check`, `git diff --check`, JSON/JS/ref/asset checks, `npm run check:production`, `npm run smoke:gallery`, protected-data diff checks, secret-pattern scan, and post-merge validation passed. Blockers: no approved production test account credentials, so live role verification, profile update, upload, and moderation mutations were not attempted. Next recommended item: G10 `qa/secrets-and-public-config-review`.
...
- Status: Complete. Branch: `chore/document-large-audio-exception`. Report: `reports/large-audio-exception.md`. PR: <https://github.com/Mochirii-Wushu/Mochirii/pull/129>. Merge commit: `2c49147d7089c47cfaf9c6606412eb7a796d258a`. Validation summary: durable content-guide note, large-audio exception report, `npm run check`, `git diff --check`, JSON/JS/ref/asset checks, `npm run check:production`, `npm run smoke:gallery`, protected-data diff checks, asset diff checks, and post-merge validation passed with the expected MP3 warning only. Blockers: none found. Next recommended item: G12 `qa/seo-social-preview-member-pages-review`.
...
- Status: Complete. Branch: `qa/seo-social-preview-member-pages-review`. Report: `reports/seo-social-preview-member-pages-review.md`. PR: <https://github.com/Mochirii-Wushu/Mochirii/pull/131>. Merge commit: `c9fc6c1f17dc36ae87ca67e391582b3210339673`. Validation summary: source/production metadata inspection, indexing decision, local metadata fix, sitemap membership check, robots review, preview image path review, `npm run check`, `git diff --check`, JSON/JS/ref/asset checks, `npm run check:production`, `npm run smoke:gallery`, protected-data diff checks, asset diff checks, and post-merge validation passed. Blockers: none found. Next recommended item: G13 `docs/supabase-cost-usage-runbook`.
