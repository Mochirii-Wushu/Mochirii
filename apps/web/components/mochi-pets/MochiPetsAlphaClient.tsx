...
import { resolveMochiPetsBridgeMessage, type MochiPetsBridgeStatus } from "@/lib/mochi-pets/bridge";
...
export function MochiPetsAlphaClient({
  gameAvailable = true,
  gamePausedMessage = "The Mochi Pets room is temporarily paused. The tester page is still open, and saved play will resume when the room is ready.",
}: MochiPetsAlphaClientProps) {
  const iframeRef = useRef<HTMLIFrameElement | null>(null);
  const [state, setState] = useState<LoadState>("loading");
  const [session, setSession] = useState<MochiPetsAlphaSession | null>(null);
  const [accessToken, setAccessToken] = useState<string | null>(null);
  const [bridgeStatus, setBridgeStatus] = useState<MochiPetsBridgeStatus>("waiting");
  const [message, setMessage] = useState("");
  const [feedback, setFeedback] = useState("");
  const [busy, setBusy] = useState(false);
  const embedUrl = useMemo(() => `${gameOrigin}/embed`, []);
  const visibleBridgeStatus: MochiPetsBridgeStatus = gameAvailable ? bridgeStatus : "error";
  ...
  const sendAuthToGame = useCallback((token: string | null) => {
    ...
    frame.postMessage({ type: "MOCHI_PETS_SIGN_OUT", protocolVersion: 1 }, gameOrigin);
  }, []);
  ...
  const refresh = useCallback(async (acknowledgeTerms = false) => {
    ...
    const result = await getMochiPetsAlphaSession({ acknowledgeTerms });
    if (!result.ok || !result.data) {
      setState("error");
      setMessage(result.message || "Mochi Pets alpha access could not be checked.");
      setBusy(false);
      return;
    }
  ...
  useEffect(() => {
    const handleGameMessage = (event: MessageEvent) => {
      ...
      const bridgeMessage = resolveMochiPetsBridgeMessage(event.data);
  ...
  const submitFeedback = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!text) return;
    setBusy(true);
    const result = await submitMochiPetsFeedback({
      category: "alpha",
      message: text,
  ...
  return (
    <section className="mochi-game-shell" aria-label="Mochi Pets alpha">
      ...
      <div className="mochi-game-preview-contract" aria-label="Mochi Pets preview contract">
      ...
      {state === "signed-out" ? (
        <div className="mochi-game-panel">
          ...
          <p>Use your Mochirii account before entering the closed Mochi Pets alpha.</p>
      ...
      {state === "blocked" ? (
        <div className="mochi-game-panel">
          ...
          <p>Your account is signed in, but it is not active on the Mochi Pets alpha tester list yet.</p>
        </div>
      ) : null}
