...
| Vercel local build logs warn `outputFileTracingRoot` and `turbopack.root` differ | Needs user decision for root-level relink; non-blocking for builds | `next build` no longer infers the wrong home-directory root. Local `vercel build --prod` can still inject an `outputFileTracingRoot` for the linked `apps/web` root and log the warning while passing. Root-level `vercel link --repo` prompted for confirmation and was cancelled rather than guessed. Dashboard Root Directory remains `apps/web`. |
...
- Local root-level Vercel relink. It needs operator confirmation at the interactive prompt.
...
- Local Vercel production build: `vercel build --prod --cwd apps/web` passed; root-link result is `NEEDS USER DECISION` because `vercel link --repo` prompted for confirmation and was cancelled. The previous `apps/web/.vercel` link was restored.
