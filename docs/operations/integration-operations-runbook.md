...
Use exact approval wording in task updates, for example: `Approve enabling Supabase Auth leaked-password protection for project deyvmtncimmcinldjyqe now.` For account-linking releases, use: `Approve enabling Supabase Auth Manual Linking for project deyvmtncimmcinldjyqe so signed-in users can link Discord, Google, Twitch, and Apple identities from Account; do not enable Facebook or Spotify identity linking.` Activating a new sign-in provider needs a separate packet naming the exact provider, Supabase project, client, callback, public presentation value, consent/callback/session smoke, brand-policy evidence, rollback, and every provider portal or Vercel mutation.
...
- Supabase work: `npm run check:supabase-security-performance`, `npm run check:supabase-edge-types`, relevant Deno tests, migration list, linked advisors, and exact-head `supabase-local-preview` evidence. Hosted `Supabase Preview` is not a required context; use `docs/operations/supabase-local-preview-ci.md` for the completed transition, local reproduction, and rollback boundary.
...
- Authentication providers: Supabase Auth is the sole OAuth broker. The source registry defines Apple, Facebook, Google, Discord, Twitch, and Spotify with exact official labels and checksum-verified local marks; the runtime sign-in allowlist exposes only configured providers, while Account identity linking independently allows only Discord, Google, Twitch, and Apple. Production provider readback remains authoritative: Facebook and Spotify stay disabled and absent from the runtime allowlist until provider configuration, callback, app-review, and provider-specific brand-approval gates pass. Do not add provider SDKs that create a second OAuth path or fetch sign-in marks from provider CDNs at runtime.
...
- Supabase pull-request validation: the source-owned `supabase-local-preview` context always reports and runs the full local database/Edge suite for owned paths. It is the required PR context; hosted `Supabase Preview` is no longer required. Supabase Automatic Branching was not changed during the transition and remains disabled and plan-locked in the dated 2026-08-01 dashboard readback. Do not change GitHub or Supabase provider settings from source work alone.
- Current local source evidence: committed candidate `bdbe9a7e8fb47646588754cf6fc1e4f6a15dc146` (tree `5ff295a4f5df0525a362dca5483243e7bfe3c9f9`) contains 53 migrations and 49 configured Functions with 31/18 JWT parity. Its unique isolated lane applied 53/53 migrations, passed 603/603 pgTAP assertions, reported zero warning-level database lint or security findings and zero unindexed foreign keys, and retained 60 fresh-empty-database `unused_index` INFO observations as non-deletion evidence. This is local validation, not hosted truth.
- GitHub ruleset snapshot (2026-08-01): strict current-head enforcement requires exactly `validate`, `validate-next`, `Vercel`, `supabase-local-preview`, `validate-theme`, and `validate-social`. Re-read the ruleset immediately before merge.
