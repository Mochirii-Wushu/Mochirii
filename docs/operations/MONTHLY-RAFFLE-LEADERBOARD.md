...
for private POST actions. The integrated source inventory is 45 functions with
28 `verify_jwt=true` and 17 false. This parity must be recalculated from the
...
3. The unavoidable automatic redeployment of all 45 functions declared in
   `supabase/config.toml`, preserving 28/17 JWT parity.
