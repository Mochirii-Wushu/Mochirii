...
This runbook describes the moderator-controlled Instagram publishing workflow for approved member Gallery images. Its intended post-deployment mode is queue review only. Manual completion is disabled, and direct API publishing remains disabled by the server activation flag until the account-confirmation checkpoint, provider restriction, human review, Graph identity, credentials, diagnostic, deployment, and action-time approval gates all pass.
...
- Employee System User: Content access only to the Page and linked Instagram
  asset plus partial Develop-app access; no full app management, ad account, or
  ad scope.
...
Graph read is the authoritative runtime identity. Current Graph discovery is
blocked by the unresolved `Account confirmation needed` checkpoint. A 60-day
token with exactly `pages_manage_posts`, `pages_read_engagement`, and
`pages_show_list` was revoked after every Graph request returned OAuthException
200 `API access blocked`. Therefore no Page-task, Instagram
identity/account-type, or Instagram Graph-ID verification is current evidence.
The former Admin System User has no assets, installed apps, or tokens and is
retained as `Mochirii Gallery Publisher Legacy`.
...
- Hosted secret values are not documented or assumed. A future release requires
  fresh name-only secret-inventory evidence, a fresh least-privilege token, and
  a successful read-only provider identity check before independently pinning
  the verified Graph ID in `INSTAGRAM_ACCOUNT_ID` and
...
Keep the Instagram and Facebook Page Website fields empty. Meta and Instagram
profile or publication copy must not place or link `mochirii.com`; technical
callback, OAuth, API, privacy-policy, and data-deletion URLs remain full URLs
