...
- Never deploy these functions manually. The existing integration redeploys the 40 functions declared in `supabase/config.toml`; post-merge readback must show every function advancing exactly once, all active, with 23 `verify_jwt=true` and 17 false.
- Before any Discord preview, verify that Vercel production is exactly bound to the merged Website commit and that the automatic Supabase deployment and 23/17 parity readback passed.
