...
- Never deploy these functions manually. The integrated source currently declares 45 functions in `supabase/config.toml`; post-merge readback must show every function advancing exactly once, all active, with 28 `verify_jwt=true` and 17 false.
- Before any Discord preview, verify that Vercel production is exactly bound to the merged Website commit and that the automatic Supabase deployment and 28/17 parity readback passed.
