...
- Never deploy these functions manually. The integrated source currently declares 49 functions in `supabase/config.toml`; post-merge readback must show every function advancing exactly once, all active, with 31 `verify_jwt=true` and 18 false.
- Before any Discord preview, verify that Vercel production is exactly bound to the merged Website commit and that the automatic Supabase deployment and 31/18 parity readback passed.
