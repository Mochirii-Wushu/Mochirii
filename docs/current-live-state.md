...
- Current `origin/main` commit verified locally on 2026-07-05: `800d3bd2294d795e709c00ce710ff4bcf93065c3` (`Refresh current readiness ledger (#382)`).
...
- Local production smoke blocker on 2026-07-05: this workstation still resets command-line TLS reads for `https://mochirii.com` and `https://social.mochirii.com` in Node fetch and curl while `https://mochirii.vercel.app` returns 200 locally. External fetches confirm `mochirii.com` and `social.mochirii.com` are live, and a short owner-approved local ProtonVPN adapter test showed the custom domains can return 200 through curl before the local smoke blocker returned. Treat `npm run check:production` failure as a local workstation/network validation blocker until VPN/security filtering is fully remediated; do not mutate DNS, Cloudflare, or Vercel from this evidence alone.
...
- Source-control evidence: Pixelfed ops `main` is current on 2026-07-05 at `0cd027723c0b9f14c28bf74f35894358fc9c9103` after PR #11 cleaned public Mochirii Social residue. Earlier ops PRs #7, #8, #9, and #10 document readiness, Spaces cutover evidence, and social media optimization policy.
...
- Media gap: DigitalOcean Spaces is configured as the primary S3-compatible media disk and a Laravel storage write/read/delete smoke passed on 2026-07-05. Broad member upload testing is still blocked until signed-in UI upload validation passes and the runtime policy is aligned with the staged target. Current live Pixelfed UI advertises JPEG/JPG/PNG/GIF up to 10 MB (`MAX_PHOTO_SIZE=10240`), while the desired policy is large friendly uploads with hard caps, optimized derivatives, and no long-term original retention.
