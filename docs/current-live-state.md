...
- Optional Lighthouse/Core Web Vitals evidence is guarded by `npm run check:lighthouse-route-matrix`; run the manual GitHub Actions Lighthouse workflow only when a production evidence packet needs fresh artifacts.
