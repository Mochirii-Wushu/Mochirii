...
- Supabase linked advisor snapshot on 2026-07-07 with CLI `2.108.0`: function inventory, migration list, and linked schema lint succeeded; security advisors returned 14 findings (`13` acknowledged service-only `rls_enabled_no_policy` info findings plus `1` leaked-password protection warning); performance advisors returned 47 info-level `unused_index` findings. No Supabase mutation or password reset was performed. See `docs/supabase-advisor-remediation-plan.md`.
