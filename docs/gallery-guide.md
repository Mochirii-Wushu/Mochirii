...
- Published member and Discord submissions are added at runtime and are not written into `apps/web/public/data/gallery.json`. The schema-v2 transition gate defaults closed, so the static Gallery remains complete while historical publication derivatives are reviewed. After an exact readiness-checked cutover, schema version 2 lists stable, credential-free thumbnail Edge URLs and resolves one bounded display-image Edge URL on demand from a stable opaque publication ID after the viewer opens.
