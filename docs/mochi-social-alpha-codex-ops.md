...
- `npm run smoke:mochi-social-tester-password-local` proves the local `/games/mochi-social` tester-password route, invalid login redirect, scoped HttpOnly cookie, unlocked iframe shell, no-real-value/`configured-preview-stub` copy, and logout path without hosted provider access.
