...
- `npm run check:mochi-social-preview-ready` reports `site.bridge-state` and `site.edge-authority` separately from hosted `site.edge-smoke`; do not treat a hosted smoke response as proof that the parent bridge, authority, or finality invariants are still wired locally.
