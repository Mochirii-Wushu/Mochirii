...
Player-facing wording belongs in `docs/mochi-pets-playtest-guide.md`
...
- `mochi_pets_progress_snapshots` stores one no-real-value alpha snapshot per Supabase user.
...
- `mochi_pets_unity_players` stores the Supabase user to UGS Custom ID/player mapping for audit and support.
- `mochi_pets_shared_pet_snapshots` stores the latest Supabase audit mirror for shared Lirabao state, not the primary runtime save.
- `mochi-pets-unity-auth` verifies Supabase auth, allowlist, and terms before returning player-scoped Unity Custom ID tokens.
- `mochi-pets-alpha-progress` loads a snapshot only for a game-server-verified, allowlisted tester who has accepted the alpha terms.
- `mochi-pets-alpha-action` remains idempotent through `mochi_pets_ledger_events.request_id`, records the action ledger, updates the legacy snapshot when the game sends `payload.state`, and mirrors shared Lirabao state on `unity.pet.state_saved`.
