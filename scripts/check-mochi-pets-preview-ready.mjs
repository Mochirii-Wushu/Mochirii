...
const credsDir = resolve(process.env.MOCHI_PETS_CREDS_DIR || defaultCredsDir());
const previewEnvPath = resolve(credsDir, process.env.MOCHI_PETS_PREVIEW_ENV_FILE || "mochi-pets-alpha-vercel-preview.local.txt");
const previewEnv = readPreviewEnvFile(previewEnvPath);
const reportPath = resolve(root, process.env.MOCHI_PETS_SITE_PREVIEW_READY_JSON || "reports/mochi-pets-preview-ready.json");
const reportMdPath = resolve(root, process.env.MOCHI_PETS_SITE_PREVIEW_READY_MD || "reports/mochi-pets-preview-ready.md");
const handoffPath = resolve(credsDir, "mochirii-mochi-pets-preview-ready.md");
const browserGateReportPath = resolve(root, process.env.MOCHI_PETS_SITE_BROWSER_GATES_JSON || "reports/mochi-pets-browser-gates.json");
const reportHygienePath = resolve(root, process.env.MOCHI_PETS_SITE_REPORT_HYGIENE_JSON || "reports/mochi-pets-report-hygiene.json");
const productionDoorwayReportPath = resolve(root, process.env.MOCHI_PETS_PRODUCTION_DOORWAY_JSON || "reports/mochi-pets-production-doorway.json");
...
addCommandRequirement("site.static-alpha", "Mochi Pets static alpha checks pass.", "node", ["scripts/check-mochi-pets-alpha.mjs"], {});
addCommandRequirement("site.bridge-state", "Mochi Pets parent bridge state self-test passes before manual browser gates.", "node", ["scripts/check-mochi-pets-bridge-state.mjs"], {});
addCommandRequirement("site.auth-bridge", "Mochi Pets auth bridge static guard keeps iframe auth access-token-only.", "node", ["scripts/check-mochi-pets-auth-bridge.mjs"], {});
addCommandRequirement("site.edge-authority", "Mochi Pets Edge authority guard passes before hosted Supabase smoke.", "node", ["scripts/check-mochi-pets-edge-authority.mjs"], {});
if (!skipNestedSelfTestCommands) {
  addCommandRequirement("site.preview-key-loader", "Mochi Pets preview publishable-key loader self-test passes without leaking key values.", "node", ["scripts/check-mochi-pets-preview-key-loader.mjs"], {});
  addCommandRequirement("site.discord-oauth-detector", "Mochi Pets Discord OAuth detector self-test passes before hosted provider checks.", "node", ["scripts/check-mochi-pets-discord-oauth-self-test.mjs"], {});
}
...
const report = {
  ...
  scope: "Mochirii Mochi Pets Alpha Preview Ready audit. This no-secret report checks the website tester-entry lane only and does not authorize provider mutations.",
  hostedChecksAllowed,
  browserGateMode,
  gameUrl,
  siteOrigin,
...
if (!report.ok) {
  console.error("Mochirii Mochi Pets Alpha Preview Ready audit is not ready:");
...
console.log(`Mochirii Mochi Pets Alpha Preview Ready audit passed. Report: ${reportPath}`);
console.log(`Markdown: ${reportMdPath}`);

function addOperatorChecklistRequirement() {
  const checklist = resolve(credsDir, "mochirii-mochi-pets-alpha-operator-next-steps.md");
  ...
  for (const snippet of [
    ...
    "NEXT_PUBLIC_MOCHI_PETS_URL",
    "MOCHI_PETS_ALPHA_EDGE_URL",
    gitState.localHead,
...
function addReportHygieneRequirement() {
  ...
  if (!report.ok) {
    add("site.report-hygiene", "fail", `Mochi Pets site report hygiene is missing or invalid: ${report.message}. Run npm run check:mochi-pets-report-hygiene after generating site reports and handoff files.`, {
...
function addProductionDoorwayRequirement() {
  ...
  if (!report.ok) {
    add("site.production-doorway-smoke", "fail", `Mochi Pets production doorway smoke report is missing or invalid: ${report.message}. Run npm run smoke:mochi-pets-production-doorway after building apps/web.`, {
  ...
  if (data?.route !== "/games/mochi-pets") failures.push("production doorway smoke report must cover /games/mochi-pets");
...
function addGameContractRequirement() {
  const failures = [];
  if (!gameUrl) failures.push("Mochi Pets game URL is required");
  ...
  if ((isHostedUrl(gameUrl) || isHostedUrl(siteOrigin)) && !hostedChecksAllowed) {
    failures.push("hosted game/site contract check requires MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true after explicit approval");
  }
  if (failures.length) {
    add("site.game-contract", "fail", failures.join("; "), { gameUrl, siteOrigin, hostedChecksAllowed });
    return;
  }
  addCommandRequirement("site.game-contract", "Game contract check passes for the configured site origin.", "node", ["scripts/check-mochi-pets-game-contract.mjs"], {
    env: {
      MOCHI_PETS_GAME_CONTRACT_URL: gameUrl,
      MOCHI_PETS_SITE_ORIGIN: siteOrigin,
    },
  });
}
...
function addEdgeSmokeRequirement() {
  ...
  if (!publishableKeyPresent) failures.push("MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY is required locally for preview smoke");
  if (isHostedUrl(functionsUrl) && !hostedChecksAllowed) {
    failures.push("hosted Supabase Edge smoke requires MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true after explicit approval");
  }
  if (failures.length) {
  ...
  addCommandRequirement("site.edge-smoke", "Supabase Edge alpha smoke passes for the preview branch.", "node", ["scripts/smoke-mochi-pets-alpha-edge.mjs"], {
    env: {
      MOCHI_PETS_ALPHA_EDGE_URL: functionsUrl,
      MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY: publishableKey.value,
    },
  });
}
...
async function addDiscordOAuthProviderRequirement() {
  ...
  if (isHostedUrl(authUrl) && !hostedChecksAllowed) {
    failures.push("hosted Discord OAuth provider check requires MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true after explicit approval");
  }
  if (failures.length) {
...
function addManualBrowserGateRequirement() {
  const envEvidenceRequested = process.env.MOCHI_PETS_SITE_BROWSER_GATES_CONFIRMED === "true" || process.env.MOCHI_PETS_SITE_BROWSER_GATES_ACCESS_MODE || manualBrowserGateChecks.some(([envName]) => process.env[envName] === "true");
  if (!envEvidenceRequested && existsSync(browserGateReportPath)) {
    addStoredManualBrowserGateRequirement();
    return;
  }

  const confirmed = process.env.MOCHI_PETS_SITE_BROWSER_GATES_CONFIRMED === "true";
  const reviewer = sanitize(process.env.MOCHI_PETS_SITE_BROWSER_GATES_REVIEWER || "");
  const browser = sanitize(process.env.MOCHI_PETS_SITE_BROWSER_GATES_BROWSER || "");
  const reviewUrl = sanitize(process.env.MOCHI_PETS_SITE_BROWSER_GATES_URL || "");
  ...
  if (confirmed && !reviewer) failures.push("MOCHI_PETS_SITE_BROWSER_GATES_REVIEWER is required");
  if (confirmed && !browser) failures.push("MOCHI_PETS_SITE_BROWSER_GATES_BROWSER is required");
  if (confirmed && !reviewUrl) failures.push("MOCHI_PETS_SITE_BROWSER_GATES_URL is required");
  ...
  if (reviewUrl && isHostedUrl(reviewUrl) && !hostedChecksAllowed) {
    failures.push("hosted browser gate confirmation requires MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true after explicit approval");
  }
...
function addStoredManualBrowserGateRequirement() {
  ...
  if (!stored.ok) {
    add("site.manual-browser-gates", "fail", `Stored browser gate report is missing or invalid: ${stored.message}. Run npm run prepare:mochi-pets-browser-gates after the approved Chrome pass.`, {
  ...
  if (reviewUrl && isHostedUrl(reviewUrl) && !hostedChecksAllowed) {
    failures.push("stored hosted browser gate evidence requires MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true after explicit approval");
  }
...
function browserGateEnvForMode(mode) {
  if (mode === "tester-password") {
    return [
      ["MOCHI_PETS_SITE_BROWSER_PASSWORD_LOCKED_OK", "tester-password locked page visible"],
      ["MOCHI_PETS_SITE_BROWSER_PASSWORD_IFRAME_ABSENT_OK", "iframe absent before tester-password unlock"],
      ["MOCHI_PETS_SITE_BROWSER_PASSWORD_INVALID_ERROR_OK", "invalid tester password shows accessible inline error"],
      ["MOCHI_PETS_SITE_BROWSER_SIGNED_OUT_BLOCKED_OK", "signed-out users see the member sign-in prompt after unlock"],
      ["MOCHI_PETS_SITE_BROWSER_NON_TESTER_BLOCKED_OK", "signed-in non-testers are blocked after unlock"],
      ["MOCHI_PETS_SITE_BROWSER_TERMS_GATE_OK", "allowlisted testers must accept terms before saved play"],
      ["MOCHI_PETS_SITE_BROWSER_IFRAME_LOADS_OK", "iframe loads after tester-password unlock and member access"],
      ["MOCHI_PETS_SITE_BROWSER_AUTH_BRIDGE_OK", "MOCHI_PETS_AUTH sends access token only after member access"],
      ["MOCHI_PETS_SITE_BROWSER_FEEDBACK_AUDIT_OK", "feedback appears in admin/audit"],
      ["MOCHI_PETS_SITE_BROWSER_NO_REAL_VALUE_OK", "no-real-value alpha copy is visible"],
      ["MOCHI_PETS_SITE_BROWSER_GAME_PRESENCE_OK", "two hosted game tabs show nearby tester presence"],
      ["MOCHI_PETS_SITE_BROWSER_ADMIN_GRANT_REVOKE_OK", "admin grant/revoke works and intended tester state is restored"],
    ];
  }

  return [
    ["MOCHI_PETS_SITE_BROWSER_SIGNED_OUT_BLOCKED_OK", "signed-out blocked"],
    ["MOCHI_PETS_SITE_BROWSER_NON_TESTER_BLOCKED_OK", "signed-in non-tester blocked"],
    ["MOCHI_PETS_SITE_BROWSER_TERMS_GATE_OK", "terms gate"],
    ["MOCHI_PETS_SITE_BROWSER_IFRAME_LOADS_OK", "iframe loads after acknowledgement"],
    ["MOCHI_PETS_SITE_BROWSER_AUTH_BRIDGE_OK", "MOCHI_PETS_AUTH sends access token only"],
    ["MOCHI_PETS_SITE_BROWSER_FEEDBACK_AUDIT_OK", "feedback appears in admin/audit"],
    ["MOCHI_PETS_SITE_BROWSER_NO_REAL_VALUE_OK", "no-real-value alpha copy is visible"],
    ["MOCHI_PETS_SITE_BROWSER_ADMIN_GRANT_REVOKE_OK", "admin grant/revoke works and intended tester state is restored"],
  ];
...
function buildReviewContext(mode, url, hostedAllowed) {
  ...
  return {
    ...
    completionPreconditions: [
      ...
      "Confirm a valid tester can load the Unity iframe and the parent sends MOCHI_PETS_AUTH only after member access.",
...
function renderMarkdown(summaryReport) {
  ...
  return `# Mochirii Mochi Pets Alpha Preview Ready Audit
...
The active production target is \`${summaryReport.siteOrigin}/games/mochi-pets\`, still behind the tester password wall. This is not a public launch. Tester password unlocks the page shell only; saved play still requires Mochirii member sign-in, tester approval, accepted terms, and valid Unity auth.
...
\`\`\`text
I approve the Mochirii hosted Preview Ready verification using MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true against the configured Fly game URL, Vercel Preview origin, and Supabase Preview Edge URL. I understand this may hit Fly/Vercel/Supabase preview resources and add usage.
\`\`\`
`;
}
...
function loadPublishableKey() {
  ...
  if (envKey) return { value: envKey, source: "env:MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY" };

  const explicitFile = process.env.MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY_FILE || "";
...
function readPreviewEnvFile(file) {
  ...
  const gameUrl = readNamedUrl(text, ["MOCHI_PETS_GAME_URL", "NEXT_PUBLIC_MOCHI_PETS_URL"]);
  const sitePreviewUrl = readNamedUrl(text, ["MOCHI_PETS_SITE_PREVIEW_URL", "NEXT_PUBLIC_SITE_URL"]);
  return {
    ...base,
    present: true,
    gameUrl,
    sitePreviewUrl,
    urlFieldsRead: [
      gameUrl ? "MOCHI_PETS_GAME_URL/NEXT_PUBLIC_MOCHI_PETS_URL" : "",
      sitePreviewUrl ? "MOCHI_PETS_SITE_PREVIEW_URL/NEXT_PUBLIC_SITE_URL" : "",
    ].filter(Boolean),
  };
}
...
function browserGateRouteFailure(value, label) {
  try {
    const parsed = new URL(value);
    if (!["http:", "https:"].includes(parsed.protocol)) return `${label} must be an http(s) URL for /games/mochi-pets`;
    const pathname = parsed.pathname.replace(/\/+$/, "") || "/";
    if (pathname !== "/games/mochi-pets") return `${label} must target /games/mochi-pets`;
    return "";
  } catch {
    return `${label} must be an absolute http(s) URL for /games/mochi-pets`;
  }
}
...
function sanitize(value) {
  return String(value || "")
    ...
    .replace(/\bMOCHI_PETS_GAME_SERVER_TOKEN\s*=\s*["']?[^ \r\n"']+/gi, "MOCHI_PETS_GAME_SERVER_TOKEN=<redacted>")
    .slice(0, 1000);
}
