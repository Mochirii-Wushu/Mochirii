...
const tempDir = mkdtempSync(join(tmpdir(), "mochirii-mochi-pets-discord-oauth-"));
...
try {
  ...
  console.log("Mochi Pets Discord OAuth provider self-test OK.");
} finally {
...
function runChecker(label, authUrl) {
  const result = spawnSync(process.execPath, [checker], {
    ...
    env: {
      ...
      MOCHI_PETS_ALPHA_AUTH_URL: authUrl,
      MOCHI_PETS_ALPHA_EDGE_URL: authUrl.replace("/auth/v1", "/functions/v1"),
      MOCHI_PETS_SITE_ORIGIN: "http://localhost:3000",
      MOCHI_PETS_GAME_CONTRACT_URL: "http://localhost:3100",
    },
  });
