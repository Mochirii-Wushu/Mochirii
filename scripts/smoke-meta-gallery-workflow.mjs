...
const validSocialJpeg = Buffer.from(
  "",
  "base64",
);
...
const nextPort = await reserveSafeLoopbackPort();
...
let serverOutput = "";
const unexpectedSupabaseServerRequests = [];
...
try {
  ...
  await waitUntilReady(server, `${nextUrl}/gallery-submit`);
  supabaseServer = await startFixtureGateway();
  ...
  assert(
    unexpectedSupabaseServerRequests.length === 0,
    `Unexpected server-side Supabase fixture requests: ${unexpectedSupabaseServerRequests.join(", ")}.`,
  );
  ...
  await stopHttpServer(supabaseServer);
...
async function runModeratorWorkflow(browser) {
  ...
  try {
    ...
    const instagramCaption = instagramCard.getByLabel("Instagram caption");
    const instagramAlt = instagramCard.getByLabel("Instagram alt text");
    const instagramPrepare = instagramCard.getByRole("button", { name: "Publish with Meta API" });
    ...
    const instagramConfirm = instagramCard.getByRole("button", { name: "Confirm Meta publish" });
    await instagramConfirm.waitFor();
    await waitForFocus(page, `[data-instagram-job-id="${instagramJobId}"] button`, "Instagram second-confirmation control", "Confirm Meta publish");

    await instagramCaption.fill("A pretty Wushu land guild moment, revised.");
    await instagramCard.getByRole("button", { name: "Publish with Meta API" }).waitFor();
    ...
    await instagramCard.getByRole("button", { name: "Publish with Meta API" }).click();
    await instagramCard.getByRole("button", { name: "Confirm Meta publish" }).click();
    ...
    const facebookCaption = facebookCard.getByLabel("Facebook Page caption");
    await facebookCaption.fill("A cupcake guild showcase from Wushu land.");
    await facebookCard.getByRole("button", { name: "Publish to Facebook Page" }).click();
    ...
    await facebookCard.getByRole("button", { name: "Publish to Facebook Page" }).waitFor();
...
async function runRepresentativeBrowser(browserType, browserName) {
  ...
  try {
    ...
    await waitForFocus(page, "#instagramOptIn", `${browserName}: Instagram opt-in keyboard focus`);
  ...
  const context = await browser.newContext({
    ...
    storageState: {
      cookies: [{
        ...
        value: `base64-${Buffer.from(JSON.stringify(syntheticSession), "utf8").toString("base64url")}`,
        domain: "127.0.0.1",
        path: "/",
        expires: expiry,
        httpOnly: false,
      ...
      origins: [{
        ...
        localStorage: [{
          ...
          value: JSON.stringify(syntheticSession),
        }],
      }],
    },
  });
...
async function assertCompactReflow(page, selector, label) {
  ...
  const nonce = await page.locator("script[nonce],style[nonce]").first()
    .evaluate((node) => node.nonce)
    .catch(() => "");
  if (nonce) {
    await page.evaluate(({ css, value }) => {
      const style = document.createElement("style");
      style.nonce = value;
      style.textContent = css;
      document.head.append(style);
    }, { css: "html{font-size:200%!important}", value: nonce });
  } else {
...
async function waitForMemberPanel(page, state, label) {
  try {
    ...
    throw new Error(`${label}: member upload panel did not become ready: ${JSON.stringify({
      ...
      pageErrors: [...state.pageErrors, ...state.consoleErrors].map(safeMessage).slice(0, 8),
      failedRequests: state.failedRequests.map(safeMessage).slice(0, 8),
    })}.`);
  }
}
...
function startFixtureGateway() {
  return new Promise((resolveServer, reject) => {
    const fixture = createHttpServer(async (request, response) => {
      const url = new URL(request.url || "/", baseUrl);
      const method = request.method || "GET";
      if (method === "GET" && url.pathname === "/auth/v1/user") {
        return sendServerJson(response, 200, syntheticUser);
      }
      if (method === "POST" && url.pathname === "/auth/v1/token") {
        return sendServerJson(response, 200, syntheticSession);
      }
      if (method === "POST" && url.pathname === "/functions/v1/list-gallery-review-queue") {
        return sendServerJson(response, 200, {
          ok: true,
          hasAccess: true,
          data: { hasAccess: true, moderatorId },
        });
      }
      if (method === "POST" && url.pathname === "/rest/v1/rpc/get_latest_official_raffle_winner") {
        return sendServerJson(response, 200, []);
      }
      if (
        ...
        unexpectedSupabaseServerRequests.push(`${method} ${url.pathname}`);
        return sendServerJson(response, 404, { message: "Synthetic Supabase server route not configured." });
      }
      return proxyFixtureRequestToNext(request, response);
    });
    fixture.once("error", reject);
    fixture.listen(port, "127.0.0.1", () => resolveServer(fixture));
  });
}

function proxyFixtureRequestToNext(request, response) {
  const upstreamUrl = new URL(request.url || "/", nextUrl);
  const headers = {
    ...request.headers,
  ...
  const upstream = forwardHttpRequest(upstreamUrl, {
  ...
  upstream.once("error", (error) => {
    if (!response.headersSent) sendServerJson(response, 502, { message: safeMessage(error) });
    else response.destroy(error);
  });
  request.pipe(upstream);
}

function sendServerJson(response, status, body) {
...
function stopHttpServer(serverToStop) {
  if (!serverToStop) return Promise.resolve();
  return new Promise((resolveStop) => serverToStop.close(resolveStop));
}
