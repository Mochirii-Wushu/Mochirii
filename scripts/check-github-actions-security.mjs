...
function parseSteps(lines, stepsIndex, end, label) {
  ...
  return starts.map((start, position) => {
    ...
    for (let index = start; index < stepEnd; index += 1) {
      ...
      if (!["name", "run", "uses", "shell", "with", "env"].includes(mapping.key)) continue;
      ...
      if (["with", "env"].includes(mapping.key)) {
        ...
        if (["run", "shell"].includes(mapping.key) && !isBlockScalar(mapping.value)) {
          for (let cursor = index + 1; cursor < stepEnd; cursor += 1) {
            ...
            throw new Error(`${label}: multiline plain ${mapping.key} scalars are outside the accepted policy grammar`);
          }
        }
        const resolved = mappingValue(lines, index, mapping);
...
const unsafeShellCanary = parseWorkflow(
  'on: push\njobs:\n  test:\n    runs-on: ubuntu-24.04\n    steps:\n      - shell: "${{ github.event.pull_request.title }}"\n        run: echo safe\n',
  "shell expression canary",
);
const unsafeShell = workflowSteps(unsafeShellCanary).some(({ step }) =>
  typeof step.shell === "string" && usesUntrustedGithubContextInRun(step.shell)
);
if (!unsafeShell) failures.push("Shell workflow-expression policy canary was not rejected.");
...
for (const name of workflowFiles) {
  ...
  for (const { jobId, index, step } of steps) {
    ...
    if (typeof step.shell === "string" && usesUntrustedGithubContextInRun(step.shell)) {
      failures.push(`${file}: job ${jobId} step ${index + 1}: do not interpolate mutable github event/ref/actor, inputs, tokens, or secrets into shell.`);
    }
  }
