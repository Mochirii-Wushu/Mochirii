...
const reportPath = resolve(root, process.env.MOCHI_PETS_SITE_REPORT_HYGIENE_JSON || "reports/mochi-pets-report-hygiene.json");
const markdownPath = resolve(root, process.env.MOCHI_PETS_SITE_REPORT_HYGIENE_MD || "reports/mochi-pets-report-hygiene.md");
const checkedAt = new Date().toISOString();

const targets = [
  { label: "production-doorway-json", path: resolve(root, "reports/mochi-pets-production-doorway.json"), required: false },
  { label: "production-doorway-md", path: resolve(root, "reports/mochi-pets-production-doorway.md"), required: false },
  { label: "preview-ready-json", path: resolve(root, "reports/mochi-pets-preview-ready.json"), required: false },
  { label: "preview-ready-md", path: resolve(root, "reports/mochi-pets-preview-ready.md"), required: false },
  { label: "browser-gates-json", path: resolve(root, "reports/mochi-pets-browser-gates.json"), required: false },
  { label: "browser-gates-md", path: resolve(root, "reports/mochi-pets-browser-gates.md"), required: false },
  { label: "operator-handoff", path: resolve(credsDir, "mochirii-mochi-pets-alpha-operator-next-steps.md"), required: false },
  { label: "preview-ready-handoff", path: resolve(credsDir, "mochirii-mochi-pets-preview-ready.md"), required: false },
  { label: "browser-gates-handoff", path: resolve(credsDir, "mochirii-mochi-pets-browser-gates.md"), required: false },
];
...
const forbiddenPatterns = [
  ...
  { label: "Mochi Pets game token assignment", pattern: /\bMOCHI_PETS_GAME_SERVER_TOKEN\s*=\s*["']?(?!<|REDACTED|redacted|placeholder)[^\s"']+/i },
...
const report = {
  ...
  checkedAt,
  scope: "Mochirii Mochi Pets no-secret report hygiene. This scans ignored local reports and Desktop Creds handoff files for obvious secret material without printing values.",
...
if (!report.ok) {
  console.error("Mochi Pets report hygiene failed.");
...
console.log(`Mochi Pets report hygiene OK (${scanned.length} no-secret artifact(s) scanned).`);
...
function renderMarkdown(reportData) {
  ...
  const failureLines = reportData.failures.length
    ...
    : "- None";
  return `# Mochi Pets Report Hygiene

Generated: ${reportData.checkedAt}

This file is intentionally no-secret. It records only hygiene status for Mochi Pets local reports and Desktop Creds handoff files.
