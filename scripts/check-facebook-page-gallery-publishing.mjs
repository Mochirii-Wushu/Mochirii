...
if (edgeCopyGuardIndex < 0 || edgeAuthIndex < 0 || edgeAuthIndex > edgeCopyGuardIndex) {
  failures.push("Facebook publish boundary must authenticate before parsing and validating publication copy.");
}

const helperCopyGuardIndex = helper.indexOf("const copyValidation = validateSocialPublicationCopy");
const helperConfigIndex = helper.indexOf(
...
if (helperCopyGuardIndex < 0 || helperConfigIndex < 0 || helperCopyGuardIndex > helperConfigIndex) {
  failures.push("Facebook publisher must reject disallowed publication copy before database or provider access.");
}

const fallbackCopyGuardIndex = helper.indexOf("const finalCopyValidation = validateSocialPublicationCopy");
const sourceLookupIndex = helper.indexOf('"gallery_facebook_page_publish_source"');
if (
  fallbackCopyGuardIndex < 0 || sourceLookupIndex < 0 ||
  fallbackCopyGuardIndex > sourceLookupIndex
) {
  failures.push("Facebook publisher must reject disallowed stored fallback copy before source or provider access.");
}
...
[
  ...
  "mochirii%252ecom",
].forEach((snippet) => includes("publication copy guard tests", publicationCopyTest, snippet));
[
  "validateSocialPublicationCopy([message])",
