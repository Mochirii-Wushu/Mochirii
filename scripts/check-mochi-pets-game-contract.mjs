const baseUrlInput = process.env.MOCHI_PETS_GAME_CONTRACT_URL || process.env.NEXT_PUBLIC_MOCHI_PETS_URL || "";
const siteOriginInput = process.env.MOCHI_PETS_SITE_ORIGIN || process.env.NEXT_PUBLIC_SITE_URL || "";

if (!baseUrlInput.trim()) {
  console.log("Mochi Pets game contract check skipped. Set MOCHI_PETS_GAME_CONTRACT_URL or NEXT_PUBLIC_MOCHI_PETS_URL to verify a local/Fly game runtime.");
  process.exit(0);
}

const baseUrl = normalizeUrl(baseUrlInput, "Mochi Pets game URL");
const siteOrigin = siteOriginInput.trim() ? normalizeOrigin(siteOriginInput, "Mochi Pets site origin") : "";

try {
  await run();
  console.log(`Mochi Pets game contract check passed for ${baseUrl}`);
  if (siteOrigin) console.log(`Allowed-origin CORS proof passed for ${siteOrigin}`);
} catch (error) {
  console.error("Mochi Pets game contract check failed:");
...
async function run() {
  ...
  assert(health.name === "Mochi Pets", "/healthz must identify Mochi Pets.");
  ...
  assert(manifest.name === "Mochi Pets", "Manifest name must stay Mochi Pets.");
  assert(manifest.slug === "mochi-pets", "Manifest slug must stay mochi-pets.");
  ...
  assert(manifest.bridge?.namespace === "MOCHI_PETS", "Manifest bridge namespace must stay MOCHI_PETS.");
  assertIncludes(manifest.bridge?.parentToGame, "MOCHI_PETS_AUTH", "Manifest parent-to-game bridge must accept MOCHI_PETS_AUTH.");
  assertIncludes(manifest.bridge?.parentToGame, "MOCHI_PETS_SIGN_OUT", "Manifest parent-to-game bridge must accept MOCHI_PETS_SIGN_OUT.");
  assertIncludes(manifest.bridge?.gameToParent, "MOCHI_PETS_READY", "Manifest game-to-parent bridge must emit MOCHI_PETS_READY.");
  assertIncludes(manifest.bridge?.gameToParent, "MOCHI_PETS_AUTH_STATE", "Manifest game-to-parent bridge must emit MOCHI_PETS_AUTH_STATE.");
  assertIncludes(manifest.bridge?.gameToParent, "MOCHI_PETS_ERROR", "Manifest game-to-parent bridge must emit MOCHI_PETS_ERROR.");
  ...
  assert(alphaStatus.edgeFunctions?.session === "mochi-pets-alpha-session", "Alpha status must expose the session Edge Function.");
  assert(alphaStatus.edgeFunctions?.unityAuth === "mochi-pets-unity-auth", "Alpha status must expose the Unity auth Edge Function.");
  assert(alphaStatus.edgeFunctions?.action === "mochi-pets-alpha-action", "Alpha status must expose the action Edge Function.");
  assert(alphaStatus.edgeFunctions?.admin === "mochi-pets-alpha-admin", "Alpha status must expose the admin Edge Function.");
  assert(alphaStatus.edgeFunctions?.feedback === "submit-mochi-pets-feedback", "Alpha status must expose the feedback Edge Function.");
  ...
  assert(String(embed.body).includes("Mochi Pets"), "/embed HTML must identify Mochi Pets.");
}
