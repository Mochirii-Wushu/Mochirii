...
const reportPath = resolve(root, process.env.MOCHI_PETS_PRODUCTION_DOORWAY_JSON || "reports/mochi-pets-production-doorway.json");
const reportMdPath = resolve(root, process.env.MOCHI_PETS_PRODUCTION_DOORWAY_MD || "reports/mochi-pets-production-doorway.md");
const port = Number(process.env.MOCHI_PETS_DOORWAY_SMOKE_PORT || 0) || randomPort();
const baseUrl = `http://127.0.0.1:${port}`;
const route = "/games/mochi-pets";
const testerPassword = `local-doorway-smoke-${Date.now()}`;
const gameOrigin = "http://127.0.0.1:1";
const cases = [];

if (!existsSync(buildIdPath)) {
  const message = "Mochi Pets production doorway smoke needs a built Next app. Run: cd apps/web && npm run build";
...
try {
  ...
  assert(!invalid.setCookie.includes("mochi_pets_tester_access="), "invalid password must not set tester cookie");
  ...
  assertIncludes(valid.setCookie, "mochi_pets_tester_access=", "valid password should set tester cookie");
  ...
  console.log(`Mochi Pets production doorway smoke passed for ${baseUrl}${route}`);
} catch (error) {
...
function startNextServer() {
  ...
  const child = spawn(process.execPath, [nextBin, "start", "--hostname", "127.0.0.1", "--port", String(port)], {
    ...
    env: {
      ...
      NEXT_PUBLIC_MOCHI_PETS_URL: gameOrigin,
...
async function writeReport({ ok, failure = "" }) {
  const report = {
    ...
    scope: "Mochirii Mochi Pets production doorway smoke. This local no-secret report verifies the tester password gate and member sign-in shell without provider mutation or hosted checks.",
    route,
    baseUrl,
...
function renderMarkdown(report) {
  const rows = report.cases
    ...
    .join("\n") || "| none | none | Not run |";
  return `# Mochi Pets Production Doorway Smoke

Generated: ${report.checkedAt}

This file is intentionally no-secret. It verifies the local production build of the Mochi Pets tester doorway only. It does not approve provider mutations, hosted checks, deployment, Enjin funding, or public launch work.
