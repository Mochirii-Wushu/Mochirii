...
const outputPath = resolve(credsDir, process.env.MOCHI_PETS_SITE_OPERATOR_CHECKLIST || "mochirii-mochi-pets-alpha-operator-next-steps.md");
const supabaseProjectRef = process.env.MOCHI_PETS_SUPABASE_PROJECT_REF || "dnxumaiooljdnbjvzbdc";
const functionsUrl = process.env.MOCHI_PETS_ALPHA_EDGE_URL || `https://${supabaseProjectRef}.supabase.co/functions/v1`;
const gameUrl = process.env.MOCHI_PETS_GAME_URL || process.env.NEXT_PUBLIC_MOCHI_PETS_URL || previewEnv.gameUrl || "https://mochi-pets-game.fly.dev";
const sitePreviewUrl = process.env.MOCHI_PETS_SITE_PREVIEW_URL || process.env.NEXT_PUBLIC_SITE_URL || previewEnv.sitePreviewUrl || "<vercel-preview-host>";
const liveSiteUrl = process.env.MOCHI_PETS_SITE_PRODUCTION_URL || process.env.MOCHI_PETS_LIVE_SITE_URL || "https://mochirii.com";
const gamePrNumber = process.env.MOCHI_PETS_GAME_PR_NUMBER || "5";
const sitePrNumber = process.env.MOCHI_PETS_SITE_PR_NUMBER || "333";
const generatedAt = new Date().toISOString();

const externalGateReport = readJson(resolve(gameRepoPath, "reports/alpha-external-gates.json"));
const gamePr = readPr("xartaiusx/mochi-pets", gamePrNumber);
...
console.log(`Wrote no-secret Mochirii Mochi Pets alpha operator checklist: ${outputPath}`);
...
function listCredentialFiles() {
  ...
  return readdirSync(credsDir, { withFileTypes: true })
    ...
    .filter((name) => /^mochi-pets-alpha|^mochirii-mochi-pets-alpha|^supabase-preview-|^enjin-|^fly-/i.test(name))
...
function renderChecklist() {
  ...
  return `# Mochirii Mochi Pets Alpha Operator Next Steps
...
- Alpha Preview Ready: live tester-password route, Unity game iframe, Supabase allowlist, terms, feedback, short-lived MOCHI_PETS_AUTH, no-real-value labels, one shared room, and shared Lirabao.
...
Target URL: ${normalizePreviewOrigin(liveSiteUrl)}/games/mochi-pets
...
- NEXT_PUBLIC_MOCHI_PETS_URL=${gameUrl}
...
\`\`\`powershell
$env:MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED="true"
$env:MOCHI_PETS_GAME_CONTRACT_URL="${gameUrl}"
$env:MOCHI_PETS_SITE_ORIGIN="${normalizePreviewOrigin(liveSiteUrl)}"
npm run check:mochi-pets-game-contract
npm run check:mochi-pets-preview-ready
\`\`\`
...
\`\`\`powershell
$env:MOCHI_PETS_GAME_CONTRACT_URL="${gameUrl}"
$env:MOCHI_PETS_SITE_ORIGIN="${siteOrigin}"
npm run check:mochi-pets-game-contract
\`\`\`
...
- MOCHI_PETS_ALPHA_TERMS_VERSION

Required Edge smoke inputs stay local:

- MOCHI_PETS_ALPHA_EDGE_URL=${functionsUrl}
- MOCHI_PETS_ALPHA_AUTH_URL=https://${supabaseProjectRef}.supabase.co/auth/v1
- MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY=<preview-supabase-publishable-key>
- MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY_FILE=${join(credsDir, `supabase-preview-${supabaseProjectRef}-api-keys.local.json`)}
- MOCHI_PETS_GAME_SERVER_TOKEN=<same-scoped-token-as-fly>
...
\`\`\`powershell
$env:MOCHI_PETS_ALPHA_EDGE_URL="${functionsUrl}"
$env:MOCHI_PETS_ALPHA_AUTH_URL="https://${supabaseProjectRef}.supabase.co/auth/v1"
$env:MOCHI_PETS_ALPHA_EDGE_PUBLISHABLE_KEY_FILE="${join(credsDir, `supabase-preview-${supabaseProjectRef}-api-keys.local.json`)}"
$env:MOCHI_PETS_GAME_SERVER_TOKEN="<same-scoped-token-as-fly>"
npm run smoke:mochi-pets-alpha-edge
npm run check:mochi-pets-report-hygiene
npm run check:mochi-pets-preview-ready # Verifies site.discord-oauth after hosted verification approval.
\`\`\`

## Manual Website Gates

Before inviting testers, verify on the approved target URL, usually ${normalizePreviewOrigin(liveSiteUrl)}/games/mochi-pets after production deployment or the Vercel preview URL during rehearsal:

1. Signed-out users are sent to sign in before entering /games/mochi-pets.
...
- For the MOCHI_PETS_AUTH gate, verify the shape of the bridge message only: the parent sends MOCHI_PETS_AUTH with a short-lived access-token field and does not send refresh tokens or provider secrets.
...
\`\`\`powershell
$env:MOCHI_PETS_SITE_BROWSER_GATES_ALLOW_HOSTED="true"
$env:MOCHI_PETS_SITE_BROWSER_GATES_ACCESS_MODE="tester-password"
$env:MOCHI_PETS_SITE_BROWSER_GATES_CONFIRMED="true"
$env:MOCHI_PETS_SITE_BROWSER_GATES_REVIEWER="<operator name>"
$env:MOCHI_PETS_SITE_BROWSER_GATES_BROWSER="<browser/version>"
$env:MOCHI_PETS_SITE_BROWSER_GATES_URL="${siteOrigin}/games/mochi-pets"
$env:MOCHI_PETS_SITE_BROWSER_GATES_NOTES="<no-secret status notes>"
...
The saved report lives at reports/mochi-pets-browser-gates.json plus C:\\Users\\xtyty\\Desktop\\Creds\\mochirii-mochi-pets-browser-gates.md. The Preview Ready audit can read that report later without restamping every browser-gate env var.
