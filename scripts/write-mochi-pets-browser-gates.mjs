...
const reportPath = resolve(root, process.env.MOCHI_PETS_SITE_BROWSER_GATES_JSON || "reports/mochi-pets-browser-gates.json");
const reportMdPath = resolve(root, process.env.MOCHI_PETS_SITE_BROWSER_GATES_MD || "reports/mochi-pets-browser-gates.md");
const handoffPath = resolve(credsDir, process.env.MOCHI_PETS_SITE_BROWSER_GATES_HANDOFF || "mochirii-mochi-pets-browser-gates.md");
const hostedChecksAllowed = process.env.MOCHI_PETS_SITE_BROWSER_GATES_ALLOW_HOSTED === "true" || process.env.MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED === "true";
const accessMode = normalizeBrowserGateMode(process.env.MOCHI_PETS_SITE_BROWSER_GATES_ACCESS_MODE || process.env.MOCHI_PETS_ALPHA_ACCESS_MODE || "tester-password");
...
const notes = sanitize(process.env.MOCHI_PETS_SITE_BROWSER_GATES_NOTES || "");
...
if (reviewUrl && isHostedUrl(reviewUrl) && !hostedChecksAllowed) {
  failures.push("hosted browser gate evidence requires MOCHI_PETS_SITE_BROWSER_GATES_ALLOW_HOSTED=true or MOCHI_PETS_SITE_PREVIEW_READY_ALLOW_HOSTED=true after explicit approval");
}

const report = {
  ok: failures.length === 0,
  checkedAt: new Date().toISOString(),
  scope: "Mochirii Mochi Pets no-secret manual browser gate evidence. This report records only pass/fail browser-gate metadata and never stores tokens, cookies, emails, raw headers, provider secrets, or screenshots.",
  accessMode,
  hostedChecksAllowed,
...
if (!report.ok) {
  console.error(`Mochi Pets browser gate evidence is incomplete: ${failures.join("; ")}`);
  console.error(`Report: ${reportPath}`);
  process.exit(1);
}

console.log(`Mochi Pets browser gate evidence passed. Report: ${reportPath}`);
...
function renderMarkdown(reportData) {
  ...
  const lines = [
    "# Mochi Pets Browser Gate Evidence",
    "",
